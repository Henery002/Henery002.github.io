...
![6](../images/linux-environment/06.png)
    ...
    **后来重装 node 后，更改了安装位置：**
    ...
    2.  软连接配置成功后可到 <font color="red">.usr/local/bin</font> 下查看：
    ...
    3.  上述安装了 node 的高版本，再使用时会带来一些意外报错问题，建议将 nodejs 降级（v7 版本）。
        ...
        由于上述方法采用 通过解压 node.tar 包、设置软连接 的方式安装 node，所以要将 <font color="red">node 重装到 v7 版本</font>时，只需要：
        ...
        4. 重新设置软连接
           <font color="red">ln -s /tmp/node-v7.9.0-linux-x64/bin/npm /usr/local/bin</font>
           <font color="red">ln -s /tmp/node-v7.9.0-linux-x64/bin/node /usr/local/bin</font>
        5. 检查是否安装成功
           node -v
           npm -v

        由于上述重装了 node，所以需要重装一下 node-sass ，然后再次运行 npm start ：
        <font color="red">npm install node-sass</font>
